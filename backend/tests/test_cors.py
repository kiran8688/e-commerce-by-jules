...
async def test_cors_preflight_restricted():
    ...
    # FastAPI CORSMiddleware returns 400 Bad Request for disallowed methods/headers in recent versions
    # or 200 without the method/header in the allow list. Let's just assert that it's correctly restricted
    # by ensuring that if it's 200, the allowed methods/headers don't contain the restricted ones.
