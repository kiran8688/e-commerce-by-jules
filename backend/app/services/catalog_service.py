...
async def get_products(db: AsyncSession, skip: int = 0, limit: int = 100) -> list[Product]:
    # Performance Optimization: Use noload() to prevent eager loading of relationships
    # that are omitted from the ProductOut schema. This avoids over-fetching
    # and reduces query times for list operations.
...
async def get_categories(db: AsyncSession, skip: int = 0, limit: int = 100) -> list[Category]:
    # Performance Optimization: Use noload() to prevent eager loading of the
    # products relationship, which is not required by the CategoryOut schema.
