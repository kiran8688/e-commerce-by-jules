...
async def get_product(db: AsyncSession, product_id: UUID) -> Product | None:
    # ⚡ Bolt Optimization: Use raiseload('*') to prevent unnecessary N+1 queries
    # from lazy='selectin' relationships (category, images, inventory, reviews)
    # since ProductOut doesn't use them.
...
async def get_categories(db: AsyncSession, skip: int = 0, limit: int = 100) -> list[Category]:
    # ⚡ Bolt Optimization: Use raiseload('*') to prevent unnecessary N+1 queries
    # from lazy='selectin' relationships since CategoryOut doesn't use them.
