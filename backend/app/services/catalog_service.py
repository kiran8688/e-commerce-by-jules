...
async def get_products(db: AsyncSession, skip: int = 0, limit: int = 100) -> list[Product]:
    # ⚡ Bolt: Use noload('*') to strip lazy="selectin" relationship loads since the
    # response schema (ProductOut) does not require related data, saving N+1 DB queries.
...
async def get_categories(db: AsyncSession, skip: int = 0, limit: int = 100) -> list[Category]:
    # ⚡ Bolt: Use noload('*') to avoid unnecessary eager loading of 'products' relationship.
    result = await db.scalars(select(Category).options(noload('*')).offset(skip).limit(limit))
    return list(result)
