...
async def get_products(db: AsyncSession, skip: int = 0, limit: int = 100) -> list[Product]:
    # ⚡ Bolt Optimization: Use noload('*') to prevent N+1 eager loading of relationships
    # (images, inventory, reviews) since ProductOut schema doesn't need them.
...
async def get_categories(db: AsyncSession, skip: int = 0, limit: int = 100) -> list[Category]:
    # ⚡ Bolt Optimization: Prevent eager loading of 'products' relationship
