...
async def get_products(db: AsyncSession, skip: int = 0, limit: int = 100) -> list[Product]:
    # Optimization: Use raiseload('*') to prevent eager loading of selectin relationships
    # (like images, inventory, reviews) when returning lists for the catalog view,
    # as they are not needed in the summary response schema.
...
async def get_categories(db: AsyncSession, skip: int = 0, limit: int = 100) -> list[Category]:
    # Optimization: Use raiseload('*') to prevent eager loading of child products
    # when only fetching category lists.
