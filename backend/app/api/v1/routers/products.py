...
async def read_products(skip: int = 0, limit: int = 100, db: AsyncSession = Depends(get_db)):
    # Use raiseload('*') to prevent N+1 queries from lazy='selectin' relationships
    # when returning a list of products where relationships are not needed in ProductOut
