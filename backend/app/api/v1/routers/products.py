...
async def read_products(skip: int = 0, limit: int = 100, db: AsyncSession = Depends(get_db)):
    # Performance Optimization: Prevent 4 unnecessary N+1 eager load queries
    # for relationships (category, images, inventory, reviews) that are lazy="selectin"
    # but not needed by the ProductOut schema.
